  ...
  useEffect(() => {
    const loadActivities = async () => {
      try {
        ...
        console.log('CategoryPage - all activities:', data);
        console.log('CategoryPage - looking for category:', categoryId);
        console.log('CategoryPage - display name:', displayCategoryName);
        
        // Debug information
        let debugInfo = `Looking for category "${categoryId}" with display name "${displayCategoryName}"`;
        console.log(debugInfo);
        console.log(`Found ${data.length} total activities`);
        ...
        if (filteredActivities.length === 0) {
          ...
          filteredActivities = data.filter(activity => {
            ...
            const match = activity.categories.some(cat => 
              cat.toLowerCase().includes(categoryId.toLowerCase()) || 
              categoryId.toLowerCase().includes(cat.toLowerCase().split(',')[0].trim())
            );
            
            console.log(`Activity "${activity.name}" - Flexible match: ${match}`);
            return match;
          });
        }
        
        // Log the results
        console.log('CategoryPage - filtered activities:', filteredActivities);
        console.log(`Found ${filteredActivities.length} matching activities`);
